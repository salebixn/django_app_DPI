...
from stock.forms import BuySellForm, SaleForm
...
def stock_sale_detail(request, pk):
    ...
    return render(request, 'stock_sale.html', context)
...
def stock_buy(request, pk):
    ...
    if form.is_valid():
        ...
        acc_stock, created = AccountStock.objects.get_or_create(account=request.user.account,
                                                                stock=stock,
...
def stock_sale(request, pk):
    ...
    if form.is_valid():
        amount = form.cleaned_data['amount']
        price = form.cleaned_data['price']
        sale_cost = price * amount
        acc_stock = AccountStock.objects.get(account=request.user.account,
                                                        stock=stock)
        
        current_cost = acc_stock.average_buy_cost * acc_stock.amount
        total_cost = current_cost + sale_cost
        total_amount = acc_stock.amount - amount
        acc_stock.amount = total_amount
        acc_stock.average_buy_cost = total_cost / total_amount
        acc_currency = AccountCurrency.objects.get(account=request.user.account,
                                                            currency=stock.currency)
        
        if acc_stock.amount < amount:
            form.add_error(None, f'На счёте недостаточно средств в валюте {stock.currency.sign}')
        else:
            acc_currency.amount = acc_currency.amount + sale_cost
            acc_stock.save()
            acc_currency.save()
